...
def setup_model_and_data(args):
    """Setup data loaders and model with optimizations"""
...
def initialize_model(model, trainer, args):
    ...
    if args.ae_model_type in ['vqvae', 'soft_vqvae']:
        ...
        # Test model
        try:
            test_input = torch.randn(2, 3, 48, 48).to(args.device)
            with torch.no_grad():
                model.eval()
            ...
            # Conservative learning rate for VQ-VAE
...
def setup_training_environment(model, args):
    ...
    print(f'Mixed precision: {use_amp}, Accumulation steps: {accumulation_steps}')
    print(f'Model parameters: {sum(x.numel() for x in model.parameters()):,}')

    if hasattr(model, 'disable_sparsity'):
        model.disable_sparsity()

    return model, training_model, use_amp, scaler, autocast_context, accumulation_steps
...
def optimized_train_loop(model, trainer, train_loader, valid_loader, args,
    ...
    if trainer and use_amp:
        ...
        trainer.recon_loss_clip = getattr(args, 'recon_loss_clip', 0)

    print(f'Starting training for {args.epochs} epochs...')

    try:
        if hasattr(trainer, 'scaler'):  # Use mixed precision training
    ...
    for epoch in range(n_epochs):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            if isinstance(batch_data, (list, tuple)):
                batch_data = [x.to(trainer.model.device, non_blocking=True) if torch.is_tensor(x) else x
                              for x in batch_data]
                    ...
                    with torch.no_grad():
                        ...
                        for val_batch in valid_loader:
                            if isinstance(val_batch, (list, tuple)):
                                val_batch = [x.to(trainer.model.device, non_blocking=True) if torch.is_tensor(x) else x
                                             for x in val_batch]
...
def evaluate_model(model, trainer, test_loader):
    ...
    with torch.no_grad():
        for batch_data in test_loader:
            if isinstance(batch_data, (list, tuple)):
                batch_data = [x.to(model.device, non_blocking=True) if torch.is_tensor(x) else x
                              for x in batch_data]
...
def train_encoder(args):
    ...
    train_loader, test_loader, valid_loader, model, trainer = setup_model_and_data(args)

    # Initialize model
    model, trainer = initialize_model(model, trainer, args)

    # Setup training environment
    model, training_model, use_amp, scaler, autocast_context, accumulation_steps = setup_training_environment(model,
                                                                                                              args)
    ...
    optimized_train_loop(
        training_model, trainer, train_loader, valid_loader, args,
        use_amp, scaler, autocast_context, accumulation_steps)
